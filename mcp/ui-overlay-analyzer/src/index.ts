...
import {
  ...
  MonitorEventsInputSchema,
  PageOpenInputSchema,
  ScreenshotInputSchema
} from "./application/dto/browser.dto.js";
...
const server = new Server(
  { name: "ar-ui-overlay-analyzer", version: "0.1.0" },
  { capabilities: { tools: {} } }
);

const browser = new BrowserService();

const okText = (text: string) => ({ content: [{ type: "text", text }] as const });
const okJson = (obj: unknown) => okText(JSON.stringify(obj, null, 2));
const errText = (text: string) => ({ isError: true, content: [{ type: "text", text }] as const });
  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description:
          "Detect element overlaps using getBoundingClientRect intersection area. Optionally returns an annotated screenshot.",
        inputSchema: FindOverlapsInputSchema
      },
      {
        name: "ar_ui_bootstrap",
        description:
          "One-shot: start browser + monitor, open URL, refresh (F5), wait, then return events + overlaps (+ optional annotated screenshot).",
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    ...
    if (name === "ar_ui_overlaps_find") {
      ...
      const top: OverlapFinding[] = findings.slice(0, input.annotateTop);
