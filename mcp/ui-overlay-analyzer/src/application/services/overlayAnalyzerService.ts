import type { OverlapFinding } from "../../domain/models.js";
import type { PlaywrightSession } from "../../infra/playwright/playwrightSession.js";
import type { FindOverlapsInputDto } from "../dto/overlaps.dto.js";

export interface FindOverlapsResultDto {
...
export class OverlayAnalyzerService {
  ...
  async findOverlaps(input: FindOverlapsInputDto): Promise<FindOverlapsResultDto> {
    const findings = await this.session.evaluate(this.findOverlapsInPage, input);
    return { findings };
  }

  async annotateOverlaps(findings: OverlapFinding[], top: number): Promise<void> {
  ...
  private readonly findOverlapsInPage = (input: FindOverlapsInputDto): OverlapFinding[] => {
    ...
    const limited =
      candidates.length <= maxElements
        ? candidates
        : candidates
            .sort((a, b) => b.area - a.area)
            .slice(0, maxElements);

    const rectRight = (r: { x: number; width: number }) => r.x + r.width;
    const rectBottom = (r: { y: number; height: number }) => r.y + r.height;

    type NodeInfo = (typeof limited)[number] & { selector: string };
    const nodes: NodeInfo[] = limited.map((n) => ({ ...n, selector: cssPath(n.el) }));
    ...
    const intersectArea = (a: NodeInfo, b: NodeInfo): number => {
  ...
  private readonly annotateInPage = (arg: { findings: OverlapFinding[] }): void => {
    ...
    arg.findings.forEach((f, idx) => {
      ...
      const makeBox = (rect: { x: number; y: number; width: number; height: number }, label: string) => {
  ...
  private readonly clearAnnotationsInPage = (): void => {
